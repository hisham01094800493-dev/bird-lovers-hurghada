...
export default function Messages() {
  ...
  return <SiteShell><section className="shell py-10 sm:py-14"><Link href="/marketplace" className="back-link"><ArrowLeft size={16} /> {isArabic ? "العودة للسوق" : "Back to marketplace"}</Link><div className="mt-8 flex items-end justify-between gap-4"><div><p className="eyebrow">{isArabic ? "الرسائل" : "Messages"} / {isArabic ? "محادثات آمنة" : "safe conversations"}</p><h1 className="page-title mt-3">{isArabic ? <>ابدأ الحديث<br /><em>بوضوح.</em></> : <>Good conversations<br /><em>start here.</em></>}</h1></div><div className="hidden items-center gap-2 text-xs text-[#799087] sm:flex"><ShieldCheck size={16} className="text-[#76a68f]" /> {isArabic ? "محادثات مصرح بها من الخادم" : "Server-authorized conversations"}</div></div><div className="messages-shell mt-9"><aside className="messages-list"><div className="border-b border-[#dce7df] px-5 py-4"><p className="eyebrow">{isArabic ? "صندوق الوارد" : "Your inbox"}</p><p className="mt-1 text-sm text-[#7c918b]">{conversations.data?.length || 0} {isArabic ? "محادثة" : "conversations"}</p></div>{conversations.data?.length ? conversations.data.map(conversation => <button key={conversation.id} className={`conversation-row ${activeId === conversation.id ? "conversation-row-active" : ""}`} onClick={() => { setSelectedId(conversation.id); navigate(`/messages?conversation=${conversation.id}`); }}><span className="avatar-placeholder"><Bird size={16} /></span><span className="min-w-0 text-left"><strong className="block truncate text-sm text-[#183b39]">{conversation.listingTitle}</strong><small className="mt-1 block text-xs text-[#82948e]">{conversation.status}</small></span></button>) : <div className="p-6 text-sm leading-6 text-[#82948e]">{isArabic ? "ابدأ من أي إعلان بالضغط على راسل البائع." : "Start from any listing by tapping “Message seller”."}</div>}</aside><section className="message-thread">{canCompose ? <><div className="border-b border-[#dce7df] px-5 py-4"><p className="font-display text-lg font-semibold">{conversations.data?.find(item => item.id === activeId)?.listingTitle || (isArabic ? "محادثة جديدة مع البائع" : "New conversation with the seller")}</p><p className="mt-1 text-xs text-[#82948e]">{isArabic ? "اكتب رسالتك بوضوح، ويمكنك إرفاق صورة أو تسجيل صوتي." : "Write your message clearly. You can also attach a photo or voice note."}</p></div><div className="thread-body">{activeId ? (messages.isLoading ? <Loader2 className="mx-auto animate-spin text-[#76a68f]" /> : messages.data?.map(message => <div key={message.id} className={`message-bubble ${message.senderId === user?.id ? "message-outgoing" : "message-incoming"}`}>{message.body && <p>{message.body}</p>}{message.attachmentPath && (message.attachmentType?.startsWith("image/") ? <a href={message.attachmentPath} target="_blank" rel="noreferrer" className="message-image-link"><img src={message.attachmentPath} alt={isArabic ? "صورة مرفقة" : "Attached image"} className="message-image" /></a> : <audio controls src={message.attachmentPath} className="message-audio" />)}<small>{message.senderName || (isArabic ? "عضو" : "Member")} · {new Date(message.createdAt).toLocaleString(isArabic ? "ar-EG" : "en-EG")}</small></div>)) : <div className="new-conversation-intro"><MessageCircle size={27} /><p>{isArabic ? "اكتب أول رسالة للبائع هنا." : "Write your first message to the seller here."}</p><small>{isArabic ? "اسأل عن حالة الطائر أو السعر أو مكان الاستلام." : "Ask about the bird’s condition, price, or pickup location."}</small></div>}</div><ChatComposer isArabic={isArabic} body={body} setBody={setBody} attachment={attachment} setAttachment={setAttachment} recording={recording} startRecording={startRecording} stopRecording={stopRecording} chooseFile={chooseFile} submit={submit} busy={send.isPending || start.isPending} /></> : <div className="empty-state m-5"><CheckCheck size={30} /><h2 className="font-display text-xl font-semibold">{isArabic ? "صندوقك جاهز." : "Your inbox is ready."}</h2><p>{isArabic ? "اختر إعلانًا لبدء محادثة آمنة." : "Choose a listing to start a secure conversation."}</p></div>}</section></div></section></SiteShell>;
}

function ChatComposer({ isArabic, body, setBody, attachment, setAttachment, recording, startRecording, stopRecording, chooseFile, submit, busy }: { isArabic: boolean; body: string; setBody: (value: string) => void; attachment: Attachment | null; setAttachment: (value: Attachment | null) => void; recording: boolean; startRecording: () => void; stopRecording: () => void; chooseFile: (event: React.ChangeEvent<HTMLInputElement>) => void; submit: (event: FormEvent) => void; busy: boolean }) {
  return <form onSubmit={submit} className="thread-composer"><div className="composer-tools"><label className="attachment-button" title={isArabic ? "إرفاق صورة" : "Attach photo"}><Paperclip size={17} /><ImagePlus size={15} /><input type="file" accept="image/png,image/jpeg,image/webp" onChange={chooseFile} hidden /></label><button type="button" className={`attachment-button ${recording ? "recording-button" : ""}`} onClick={recording ? stopRecording : startRecording} title={recording ? (isArabic ? "إيقاف التسجيل" : "Stop recording") : (isArabic ? "تسجيل صوتي" : "Record voice note")}>{recording ? <Square size={16} /> : <Mic size={17} />}</button></div><div className="composer-field"><Textarea value={body} onChange={event => setBody(event.target.value)} placeholder={isArabic ? "اكتب رسالتك هنا… اسأل عن الحالة أو السعر أو مكان الاستلام" : "Write your message here… ask about condition, price, or pickup"} className="min-h-14 resize-none rounded-xl border-[#dce7df] bg-[#fbfcfa]" />{attachment && <div className="attachment-preview">{attachment.type.startsWith("image/") ? <img src={attachment.data} alt="" /> : <Mic size={15} />}<span>{attachment.name}</span><button type="button" onClick={() => setAttachment(null)} aria-label={isArabic ? "إزالة المرفق" : "Remove attachment"}><X size={15} /></button></div>}<small className="composer-hint">{recording ? (isArabic ? "جاري التسجيل… اضغط الميكروفون للإيقاف" : "Recording… tap the microphone to stop") : (isArabic ? "يمكنك الكتابة أو إرفاق صورة أو إرسال تسجيل صوتي" : "You can type, attach a photo, or send a voice note")}</small></div><Button type="submit" className="cta-primary h-12" disabled={busy || recording}>{busy ? <Loader2 className="animate-spin" size={16} /> : <Send size={16} />} {isArabic ? "إرسال" : "Send"}</Button></form>;
}

...
function fallbackImage(title: string) {
  ...
  if (normalized.includes("cage") || normalized.includes("قفص")) return "/images/cage-gold.jpg";
  if (normalized.includes("food") || normalized.includes("غذاء")) return "/images/bird-seed.jpg";
  if (normalized.includes("macaw") || normalized.includes("مكاو")) return "/images/hurghada-parrot-hero.jpg";
  ...
  return <>
    <div className="space-y-3">
      <div className="group relative overflow-hidden rounded-3xl bg-[#eef5ed]" onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
        <button type="button" className="block w-full cursor-zoom-in" onClick={() => setLightbox(true)} aria-label="Open photo gallery"><img src={current.url} alt={current.alt || title} className="aspect-[4/3] w-full object-cover" onError={event => { event.currentTarget.onerror = null; event.currentTarget.src = fallbackImage(title); }} /></button>

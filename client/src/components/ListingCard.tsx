...
export default function ListingCard({ listing, compact = false }: { listing: ListingCardData; compact?: boolean }) {
  ...
  return <article className={`listing-card ${compact ? "listing-card-compact" : ""}`}><Link href={`/listing/${listing.id}`} className="group block"><div className="listing-image-wrap">{listing.coverImage ? <img src={listing.coverImage} alt={title} className="listing-image" onError={event => { event.currentTarget.onerror = null; event.currentTarget.src = fallbackImage(listing); }} /> : <img src={fallbackImage(listing)} alt={title} className="listing-image" />}<div className="absolute left-3 top-3 flex gap-2"><span className="pill pill-light">{category}</span>{listing.exchangeAvailable && <span className="pill pill-accent"><Repeat2 size={12} /> {t("exchange")}</span>}</div><div className="absolute bottom-3 left-3 rounded-lg bg-[#183b39]/90 px-3 py-1.5 text-sm font-semibold text-white">{price} <span className="text-[10px] font-medium opacity-70">{isArabic ? "ج.م" : listing.currency}</span></div></div></Link><div className="p-4"><div className="flex items-start justify-between gap-3"><Link href={`/listing/${listing.id}`} className="min-w-0"><h3 className="line-clamp-2 font-display text-lg font-semibold leading-tight text-[#183b39] group-hover:text-[#d26246]">{title}</h3>{!isArabic && listing.titleAr && <p dir="rtl" className="mt-1 line-clamp-1 text-sm text-[#83918c]">{listing.titleAr}</p>}</Link><Button variant="ghost" size="icon" className={`shrink-0 rounded-full ${status.data ? "text-[#d26246]" : "text-[#9aaca5]"}`} onClick={event => { event.preventDefault(); if (!isAuthenticated) { toast.info(t("loginToSave")); return; } toggle.mutate({ listingId: listing.id }); }} aria-label={t("saveListing")}><Heart size={18} fill={status.data ? "currentColor" : "none"} /></Button></div><div className="mt-4 flex items-center justify-between gap-2 text-xs text-[#7a9089]"><span className="flex items-center gap-1"><MapPin size={13} /> {listing.location}</span><span className="flex items-center gap-1"><Tag size={13} /> {listing.negotiable ? t("negotiable") : t("fixedPrice")}</span></div></div></article>;
}

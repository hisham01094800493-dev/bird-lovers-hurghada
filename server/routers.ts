...
import { addMessage, canReviewCompletedListing, createCommunityComment, createConversationMessage, createCustomNotifications, createLocalUser, createLostFoundReport, createNotification, createPriceGuideItem, createReport, createReview, deletePriceGuideItem, getAdminStats, getConversation, getDb, getListingById, hasCommunityLike, listAdminUsers, updateAdminUser, getListingSeller, getNotificationPreferences, getProfile, getUserByEmail, isFavorite, listAppUpdates, listCategories, listCommunityComments, listCommunityPosts, listConversations, listFavorites, listListingImages, listListings, listLostFoundReports, listMessageAttachments, listMessages, listModerationPosts, listMyListings, listNotifications, listOpenReports, listPendingListings, listPendingPriceGuideDrafts, listPriceGuide, moderateCommunityPost, approvePriceGuideDraft, rejectPriceGuideDraft, moderateListing, publishAppUpdate, reorderListingImages, requestContactVerification, resolveReport, resolveLostFoundReport, toggleCommunityLike, unreadNotificationCount, updateNotificationPreferences, updatePriceGuideItem, updateProfile, upsertUser } from "./db";
...
export const appRouter = router({
  ...
  community: router({
    ...
    comments: publicProcedure.input(z.object({ postId: z.number().int().positive() })).query(({ input }) => listCommunityComments(input.postId)),
    liked: protectedProcedure.input(z.object({ postId: z.number().int().positive() })).query(({ ctx, input }) => hasCommunityLike(ctx.user.id, input.postId)),
    toggleLike: protectedProcedure.input(z.object({ postId: z.number().int().positive() })).mutation(({ ctx, input }) => toggleCommunityLike(ctx.user.id, input.postId)),
    comment: protectedProcedure.input(z.object({ postId: z.number().int().positive(), body: z.string().trim().min(1).max(1000) })).mutation(({ ctx, input }) => createCommunityComment(ctx.user.id, input.postId, input.body)),
    create: protectedProcedure.input(z.object({ category: z.enum(["care", "nutrition", "health", "breeding", "general", "other"]), title: z.string().trim().min(4).max(180), body: z.string().trim().min(10).max(5000), imageData: z.string().max(30_000_000).optional() })).mutation(async ({ ctx, input }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database is not available" }); let imagePath: string | null = null; if (input.imageData) { const match = input.imageData.match(/^data:(image\/(?:jpeg|png|webp));base64,(.+)$/); if (!match) throw new TRPCError({ code: "BAD_REQUEST", message: "استخدم صورة JPG أو PNG أو WebP" }); try { const buffer = Buffer.from(match[2], "base64"); if (buffer.length > 20_000_000) throw new Error("large"); await sharp(buffer, { failOn: "error" }).metadata(); const normalized = await sharp(buffer).rotate().resize({ width: 1800, height: 1800, fit: "inside", withoutEnlargement: true }).webp({ quality: 82 }).toBuffer(); imagePath = (await storagePut(`community/${ctx.user.id}/post-${Date.now()}.webp`, normalized, "image/webp")).url; } catch { throw new TRPCError({ code: "BAD_REQUEST", message: "تعذر قراءة الصورة، جرّب صورة JPG أو PNG عادية" }); } } const [created] = await db.insert(communityPosts).values({ authorId: ctx.user.id, category: input.category, title: input.title, body: input.body, imagePath }); return { id: Number(created.insertId) }; }),
  }),
  ...
  admin: router({
    ...
    approvePriceDraft: adminProcedure.input(z.object({ draftId: z.number().int().positive() })).mutation(({ ctx, input }) => approvePriceGuideDraft(ctx.user.id, input.draftId)),
    rejectPriceDraft: adminProcedure.input(z.object({ draftId: z.number().int().positive(), reason: z.string().max(500).default("") })).mutation(({ ctx, input }) => rejectPriceGuideDraft(ctx.user.id, input.draftId, input.reason)),

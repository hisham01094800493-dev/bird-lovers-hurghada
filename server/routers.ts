...
import { addMessage, canReviewCompletedListing, createConversationMessage, createCustomNotifications, createLocalUser, createNotification, createPriceGuideItem, createReport, createReview, deletePriceGuideItem, getAdminStats, getConversation, getDb, getListingById, listAdminUsers, updateAdminUser, getListingSeller, getNotificationPreferences, getProfile, getUserByEmail, isFavorite, listAppUpdates, listCategories, listCommunityPosts, listConversations, listFavorites, listListingImages, listListings, listMessageAttachments, listMessages, listModerationPosts, listMyListings, listNotifications, listOpenReports, listPendingListings, listPendingPriceGuideDrafts, listPriceGuide, moderateCommunityPost, approvePriceGuideDraft, rejectPriceGuideDraft, moderateListing, publishAppUpdate, reorderListingImages, requestContactVerification, resolveReport, unreadNotificationCount, updateNotificationPreferences, updatePriceGuideItem, updateProfile, upsertUser } from "./db";
...
export const appRouter = router({
  ...
  community: router({
    ...
    create: protectedProcedure.input(z.object({ category: z.enum(["care", "nutrition", "health", "breeding", "general", "other"]), title: z.string().trim().min(4).max(180), body: z.string().trim().min(10).max(5000), imageData: z.string().max(12_000_000).optional() })).mutation(async ({ ctx, input }) => { const db = await getDb(); if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Database is not available" }); let imagePath: string | null = null; if (input.imageData) { const match = input.imageData.match(/^data:(image\/(?:jpeg|png|webp));base64,(.+)$/); if (!match) throw new TRPCError({ code: "BAD_REQUEST", message: "Use a JPG, PNG, or WebP image" }); try { const buffer = Buffer.from(match[2], "base64"); if (buffer.length > 8_000_000) throw new Error("large"); const metadata = await sharp(buffer, { failOn: "error" }).metadata(); if (!metadata.width || !metadata.height || metadata.width < 320 || metadata.height < 320) throw new Error("small"); const normalized = await sharp(buffer).rotate().resize({ width: 1800, height: 1800, fit: "inside", withoutEnlargement: true }).webp({ quality: 82 }).toBuffer(); imagePath = (await storagePut(`community/${ctx.user.id}/post-${Date.now()}.webp`, normalized, "image/webp")).url; } catch { throw new TRPCError({ code: "BAD_REQUEST", message: "الصورة يجب أن تكون صالحة وبأبعاد لا تقل عن 320×320" }); } } const [created] = await db.insert(communityPosts).values({ authorId: ctx.user.id, category: input.category, title: input.title, body: input.body, imagePath }); return { id: Number(created.insertId) }; }),
  }),
